...
export const WORLD_CHUNKS = 27; // 27x32 = 864 tiles per side, making map generation extremely fast and responsive without any freeze or crash
...
export class World implements PlayMap {
  ...
  recruiter: { x: number; y: number } | null = null;
  ...
  private buildCentralCity(cx: number, cy: number) {
    ...
    // Portal de Continentes (kind 22) — no eixo sul-central da praça
    const pcx = cx, pcy = cy + 8;
    if (this.inBounds(pcx, pcy)) this.tiles[this.idx(pcx, pcy)].kind = 22;
    if (this.inBounds(pcx - 1, pcy)) this.tiles[this.idx(pcx - 1, pcy)].kind = 11;
    if (this.inBounds(pcx + 1, pcy)) this.tiles[this.idx(pcx + 1, pcy)].kind = 11;
    ...
    // Recrutador de Mercenários NPC (kind 23) — simétrico ao ferreiro a noroeste (cx-8..cx-5, cy-7..cy-4)
    const mrx = cx - 6, mry = cy - 5;
    if (this.inBounds(mrx, mry)) this.tiles[this.idx(mrx, mry)].kind = 23;
    this.recruiter = { x: mrx * TILE + TILE / 2, y: mry * TILE + TILE / 2 };
    ...
    // Build the magnificent Eternal Temple of Asterion next to the city
    this.buildEternalTemple(cx, cy);
  }

  templeEntrance: { x: number; y: number } | null = null;
  private buildEternalTemple(cx: number, cy: number) {
    const tx = cx + 25;
    const ty = cy;
    const size = 9;
    const half = Math.floor(size / 2);

    // Outline walls and pave the temple
    for (let yy = -half; yy <= half; yy++) {
      for (let xx = -half; xx <= half; xx++) {
        const x = tx + xx;
        const y = ty + yy;
        if (!this.inBounds(x, y)) continue;
        const dist = Math.max(Math.abs(xx), Math.abs(yy));
        const t = this.tiles[this.idx(x, y)];
        if (dist === half) {
          t.kind = 14; // wall
          t.biome = 1;
        } else {
          t.kind = 13; // paved floor
          t.biome = 1;
        }
      }
    }

    // Four beautiful corner steel pillars to anchor the sanctuary
    for (const [px, py] of [
      [tx - half, ty - half],
      [tx + half, ty - half],
      [tx - half, ty + half],
      ...
      if (this.inBounds(px, py)) this.tiles[this.idx(px, py)].kind = 11;
    }

    // Gate entry facing West (towards the main city)
    const gx = tx - half;
    const gy = ty;
    if (this.inBounds(gx, gy)) this.tiles[this.idx(gx, gy)].kind = 10; // Open gate

    // Carve a direct paved pathway from the city's East gate (cx + R, cy) to the Temple gate (gx, gy)
    const startX = cx + 14; // City East Gate (R = 14)
    for (let x = startX + 1; x < gx; x++) {
      if (this.inBounds(x, gy)) {
        this.tiles[this.idx(x, gy)].kind = 13; // pave path
        // Place beautiful safety torches along the path!
        if (x % 3 === 0 && this.inBounds(x, gy - 1)) {
          this.tiles[this.idx(x, gy - 1)].kind = 8;
        }
      }
    }

    // At the temple center: Tile kind 21 is our special TEMPLE PORTAL / STAIRCASE
    if (this.inBounds(tx, ty)) {
      this.tiles[this.idx(tx, ty)].kind = 21;
    }

    // Symmetrical torches and fountains inside
    if (this.inBounds(tx, ty - 2)) this.tiles[this.idx(tx, ty - 2)].kind = 8;
    if (this.inBounds(tx, ty + 2)) this.tiles[this.idx(tx, ty + 2)].kind = 8;
    if (this.inBounds(tx - 2, ty - 2)) this.tiles[this.idx(tx - 2, ty - 2)].kind = 15; // Fountain
    if (this.inBounds(tx + 2, ty - 2)) this.tiles[this.idx(tx + 2, ty - 2)].kind = 15; // Fountain
    if (this.inBounds(tx - 2, ty + 2)) this.tiles[this.idx(tx - 2, ty + 2)].kind = 15; // Fountain
    if (this.inBounds(tx + 2, ty + 2)) this.tiles[this.idx(tx + 2, ty + 2)].kind = 15; // Fountain

    this.templeEntrance = { x: tx * TILE + TILE / 2, y: ty * TILE + TILE / 2 };
  }
}
...
export class Player {
  ...
  atk() {
    ...
    // Divine Ascension multiplier (20% per rebirth level)
    const divineMult = 1 + ((this as any).divineAscensions || 0) * 0.20;
    total = Math.floor(total * divineMult);

    // Refinement system: Neptune's Trident relic (+10% ATK)
    const relics = (this as any).discoveredRelics || [];
    if (relics.includes("🔱 Tridente de Netuno")) {
      total = Math.floor(total * 1.10);
    }
    // Divine Ascension Perks: Fúria Divina (1+ ascensions, +5% extra damage multiplier)
    if (((this as any).divineAscensions || 0) >= 1) {
      total = Math.floor(total * 1.05);
    }
  ...
  def() {
    ...
    // Divine Ascension multiplier (20% per rebirth level)
    const divineMult = 1 + ((this as any).divineAscensions || 0) * 0.20;
    totalDef = Math.floor(totalDef * divineMult);

    // Refinement system: Ether Pearl relic (+10% DEF)
    const relics = (this as any).discoveredRelics || [];
    if (relics.includes("🐚 Pérola de Éter")) {
      totalDef = Math.floor(totalDef * 1.10);
    }
    // Divine Ascension Perks: Dobra Temporal (3+ ascensions, +5% extra defense multiplier)
    if (((this as any).divineAscensions || 0) >= 3) {
      totalDef = Math.floor(totalDef * 1.05);
    }

    // Ascension Perk: Casca de Titã (+5% defense per point)
    const cascaPts = (this as any).ascensionPerks?.casca || 0;
    if (cascaPts > 0) {
      totalDef = Math.floor(totalDef * (1 + cascaPts * 0.05));
    }
  ...
  gainXp(amount: number, log: (m: string, c?: string) => void) {
    ...
    let mult = this.runeBonuses?.xpMult ?? 1;
    // Refinement system: Destiny Compass relic (+15% XP)
    const relics = (this as any).discoveredRelics || [];
    if (relics.includes("🧭 Bússola do Destino")) {
      mult *= 1.15;
    }
