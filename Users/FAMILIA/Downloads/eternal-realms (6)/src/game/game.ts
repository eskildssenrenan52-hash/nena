...
function damageEnemy(g: GameState, e: Enemy, dmg: number) {
  ...
  const comboDmgMult = 1 + Math.min(0.50, (g.comboCount || 0) * 0.02);
  const perkDmgMult = 1 + (g.ambitious?.ascensionPerks?.garras || 0) * 0.05;
  const scaledDmg = modDmg * (g.player.tempDmgMult ?? 1.0) * comboDmgMult * perkDmgMult;
...
export function recalculateStats(g: GameState) {
  ...
  const p = g.player;
  (p as any).ascensionPerks = g.ambitious?.ascensionPerks;
  ...
  // Velocidade base * bônus de nível (+1% por nível) * multiplicador de runas (base 3.2) * bônus de combo (máx +20%) * bônus de perk de Ascensão (fluxo)
  const comboSpeedMult = 1 + Math.min(0.20, (g.comboCount || 0) * 0.01);
  const perkSpeedMult = 1 + (g.ambitious?.ascensionPerks?.fluxo || 0) * 0.03;
  p.speed = 3.2 * (1 + Math.max(0, p.level - 1) * 0.01) * (rb.moveSpeedMult || 1) * comboSpeedMult * perkSpeedMult;
...
export function update(g: GameState, dt: number, input: Input, canvasW: number, canvasH: number) {
  ...
  if (!inArena(g)) {
    ...
    for (let i = 0; i < g.enemies.length; i++) {
      if (!g.enemies[i].isBoss) nonBossCount++;
    }
    if (!inSafeZone && !isApexFloor && g.spawnTimer <= 0 && nonBossCount < enemyCap) {
    ...
    if (!inSafeZone && !isApexFloor && g.bossTimer <= 0) {
      let bossCount = 0;
      for (let i = 0; i < g.enemies.length; i++) {
        if (g.enemies[i].isBoss) bossCount++;
      }
      if (bossCount < 2) spawnEnemyNear(g, p.pos.x, p.pos.y, true);
  ...
  for (let i = 0; i < projectiles.length; i++) {
    ...
    if (keep) {
      projectiles[prWrite++] = pr;
    }
  }
  projectiles.length = prWrite;

  const floaters = g.floaters;
  let fWrite = 0;
  ...
  floaters.length = fWrite;

  // particles
  if (g.particles) {
    const particles = g.particles;
    let pWrite = 0;
    const maxParticles = 250; // Performance safeguard limit
    const pLen = Math.min(particles.length, maxParticles);
