...
import { type GameState, recalculateStats } from "./game";
...
export interface Pet {
  id: string;
  name: string;
  type: "egg" | "fire" | "ice" | "lightning" | "healing" | "hybrid";
  level: number;
  xp: number;
  maxXp: number;
  ...
  color: string;
  isHybrid?: boolean;
  hybridType?: "fire_ice" | "lightning_healing" | "fire_lightning" | "ice_healing";
}
...
export interface AmbitiousState {
  ...
  ascensionPerks?: {
    garras: number; // +5% Crit Dmg per point
    casca: number;  // +5% Defense per point
    fluxo: number;  // +3% Speed per point
    furia: number;  // +10% Ult Charge per point
  };
}
...
export function updateAmbitiousSystems(g: GameState, dt: number) {
  ...
  if (!s.ascensionPerks) {
    s.ascensionPerks = { garras: 0, casca: 0, fluxo: 0, furia: 0 };
  }
  if (!s.divineEmbers) {
    s.divineEmbers = 0;
  }
  ...
  if (s.pets.length === 0) {
    s.pets.push({ id: "p1", name: "Ovo de Draguinho", type: "egg", level: 1, xp: 0, maxXp: 100, rarity: "Raro", active: false, color: "#f87171" });
  }
  const p = g.player;
  ...
  // Keep slowly passive charging, speeded up by high kill combo and Fúria Recorrente perk
  if (s.ultimateGauge < s.ultimateMax) {
    const perkUltMult = 1 + (s.ascensionPerks?.furia || 0) * 0.10;
    const comboUltMult = 1 + Math.min(0.50, (g.comboCount || 0) * 0.02);
    s.ultimateGauge = Math.min(s.ultimateMax, s.ultimateGauge + dt * 1.5 * perkUltMult * comboUltMult);
  }
  ...
  if (activePet && activePet.type !== "egg") {
    ...
    if (Math.random() < 0.02) {
      if (activePet.type === "healing") {
      ...
      } else if (activePet.type === "hybrid") {
        // Mutated Hybrid Pet Actions!
        const hType = activePet.hybridType || "fire_ice";
        const levelMult = 1 + activePet.level * 0.1;
        
        if (hType === "fire_ice") {
          // Cast Glacial Fire Strike
          let nearest: Enemy | null = null;
          let minDist = 250;
          for (const e of g.enemies) {
            const d = Math.hypot(e.pos.x - p.pos.x, e.pos.y - p.pos.y);
          ...
          if (nearest) {
            const petDmg = Math.floor(25 * levelMult);
            nearest.hp = Math.max(0, nearest.hp - petDmg);
            nearest.hitFlash = 0.25;
            g.floaters.push({
              x: nearest.pos.x,
              y: nearest.pos.y - 12,
              text: `❄️🔥 ${petDmg} (Fogo Glacial)`,
              color: "#38bdf8",
              life: 1.2,
              vy: -22,
            });
            spawnAmbitiousParticles(g, nearest.pos.x, nearest.pos.y, "#f97316", 5);
            spawnAmbitiousParticles(g, nearest.pos.x, nearest.pos.y, "#60a5fa", 5);
          }
        } else if (hType === "lightning_healing") {
          // Heal + Lightning chain
          const heal = Math.floor(12 * levelMult);
          p.hp = Math.min(p.maxHp, p.hp + heal);
          g.floaters.push({ x: p.pos.x + 12, y: p.pos.y - 12, text: `💚 +${heal}`, color: "#4ade80", life: 1.0, vy: -18 });
          ...
          if (nearest) {
            const petDmg = Math.floor(18 * levelMult);
            nearest.hp = Math.max(0, nearest.hp - petDmg);
            nearest.hitFlash = 0.2;
            g.floaters.push({ x: nearest.pos.x, y: nearest.pos.y - 12, text: `⚡ +💚 ${petDmg}`, color: "#c084fc", life: 1.0, vy: -22 });
            spawnAmbitiousParticles(g, nearest.pos.x, nearest.pos.y, "#a855f7", 8);
          }
        } else if (hType === "fire_lightning") {
          // Massive explosive strike
          let nearest: Enemy | null = null;
          let minDist = 250;
          for (const e of g.enemies) {
            const d = Math.hypot(e.pos.x - p.pos.x, e.pos.y - p.pos.y);
          ...
          if (nearest) {
            const petDmg = Math.floor(32 * levelMult);
            nearest.hp = Math.max(0, nearest.hp - petDmg);
            nearest.hitFlash = 0.3;
            g.floaters.push({ x: nearest.pos.x, y: nearest.pos.y - 12, text: `💥⚡ ${petDmg} (Trovoada)`, color: "#f97316", life: 1.2, vy: -25 });
            spawnAmbitiousParticles(g, nearest.pos.x, nearest.pos.y, "#ea580c", 10);
          }
        } else if (hType === "ice_healing") {
          // Heal + shield freeze
          const heal = Math.floor(15 * levelMult);
          p.hp = Math.min(p.maxHp, p.hp + heal);
          g.floaters.push({ x: p.pos.x + 12, y: p.pos.y - 12, text: `💚 +${heal}`, color: "#4ade80", life: 1.0, vy: -18 });
          
          const shieldAmt = Math.floor(6 * levelMult);
          p.shield = Math.min(p.maxShield, (p.shield || 0) + shieldAmt);
          g.floaters.push({ x: p.pos.x - 12, y: p.pos.y - 12, text: `🛡️ +${shieldAmt}`, color: "#38bdf8", life: 1.0, vy: -15 });
          spawnAmbitiousParticles(g, p.pos.x, p.pos.y, "#38bdf8", 6);
        }
        ...
        if (activePet.xp >= activePet.maxXp) {
          ...
          logMsg(g, `🐾 HÍBRIDO UP: ${activePet.name} subiu para o Nível ${activePet.level}!`, "#a855f7");
        }
      }
    }
  }
...
// ---------------- 100x COMPLEXITY GAMEPLAY SYSTEMS ----------------

/**
 * Synthesizes two level 5+ pets of different elements into a powerful Mutated Hybrid!
 * Consumes both parent pets and outputs a Level 1 Epic/Legendary Hybrid.
 */
export function synthesizeHybridPet(g: GameState, id1: string, id2: string): boolean {
  const s = g.ambitious;
  if (!s) return false;

  const p1 = s.pets.find(p => p.id === id1);
  const p2 = s.pets.find(p => p.id === id2);

  if (!p1 || !p2) {
    logMsg(g, "❌ Mascote não encontrado para sintetização!", "#ef4444");
    return false;
  }
  if (p1.id === p2.id) {
    logMsg(g, "❌ Você deve selecionar dois mascotes diferentes!", "#ef4444");
    return false;
  }
  if (p1.level < 5 || p2.level < 5) {
    logMsg(g, "❌ Ambos os mascotes precisam ser no mínimo Nível 5!", "#ef4444");
    return false;
  }

  // Determine hybrid type
  const types = [p1.type, p2.type];
  let hType: "fire_ice" | "lightning_healing" | "fire_lightning" | "ice_healing" = "fire_ice";
  let hName = "Dragão Glacial Mutante";
  let hColor = "#38bdf8";

  if (types.includes("fire") && types.includes("ice")) {
    hType = "fire_ice";
    hName = "Dragão Glacial";
    hColor = "#60a5fa";
  } else if (types.includes("lightning") && types.includes("healing")) {
    hType = "lightning_healing";
    hName = "Espírito da Tempestade Curadora";
    hColor = "#c084fc";
  } else if (types.includes("fire") && types.includes("lightning")) {
    hType = "fire_lightning";
    hName = "Fênix do Trovão";
    hColor = "#fb923c";
  } else if (types.includes("ice") && types.includes("healing")) {
    hType = "ice_healing";
    hName = "Tartaruga de Jade Anciã";
    hColor = "#2dd4bf";
  } else {
    // Default fallback combo
    hType = "fire_ice";
    hName = "Quimera Elemental";
    hColor = "#f43f5e";
  }

  // Delete parents
  s.pets = s.pets.filter(p => p.id !== id1 && p.id !== id2);

  // Generate hybrid pet
  const newId = `hybrid_${Date.now()}`;
  const hybridPet: Pet = {
    id: newId,
    name: `🐾 ${hName}`,
    type: "hybrid",
    ...
    xp: 0,
    maxXp: 150,
    rarity: Math.random() < 0.2 ? "Lendário" : "Épico",
    active: true,
    color: hColor,
    isHybrid: true,
    hybridType: hType
  ...
  s.pets.push(hybridPet);
  logMsg(g, `🎉 MUTAÇÃO DE ALTAR SUCESSO: Você sintetizou o híbrido ${hybridPet.name}!`, "#a855f7");
...
/**
 * Purchases an Ascension Perk using Divine Embers.
 * Progressive Ember costs: current perk level + 1 embers. Max level 5.
 */
export function buyAscensionPerk(g: GameState, perk: "garras" | "casca" | "fluxo" | "furia"): boolean {
  const s = g.ambitious;
  if (!s) return false;
  ...
  const curLvl = s.ascensionPerks[perk] || 0;
  if (curLvl >= 5) {
    logMsg(g, "❌ Esta perk já está no nível máximo (Nível 5)!", "#ef4444");
    return false;
  }

  const cost = curLvl + 1;
  if (s.divineEmbers < cost) {
    logMsg(g, `❌ Cinzas Divinas insuficientes! Necessário: ${cost} Cinzas.`, "#ef4444");
    return false;
  }

  s.divineEmbers -= cost;
  s.ascensionPerks[perk] = curLvl + 1;
  
  const perkLabels: Record<string, string> = {
    garras: "Garras Divinas (+5% Dano Crítico por nível)",
    casca: "Casca de Titã (+5% Defesa por nível)",
    fluxo: "Fluxo Sideral (+3% Velocidade por nível)",
    furia: "Fúria Recorrente (+10% Recarga do Supremo por nível)"
  };

  logMsg(g, `✨ PERK MELHORADA: ${perkLabels[perk]} subiu para o Nível ${s.ascensionPerks[perk]}!`, "#ffd700");
  
  // Trigger stat recalculation so defensive stats, speed, etc are applied immediately
  recalculateStats(g);
  return true;
}

/**
 * Purchases a new Pet Egg.
 */
export function buyPetEgg(g: GameState, currency: "gold" | "embers"): boolean {
  const s = g.ambitious;
  if (!s) return false;

  if (currency === "gold") {
    if (g.player.gold < 15000) {
      logMsg(g, "❌ Ouro insuficiente! O ovo custa 15.000g.", "#ef4444");
      return false;
    }
    g.player.gold -= 15000;
  } else {
    if (s.divineEmbers < 5) {
      logMsg(g, "❌ Cinzas Divinas insuficientes! O ovo custa 5 🔥.", "#ef4444");
      return false;
    }
    s.divineEmbers -= 5;
  }
  ...
  const eggNames = ["Ovo Misterioso", "Ovo Cósmico", "Ovo Elemental", "Ovo Antigo"];
  const eggName = eggNames[Math.floor(Math.random() * eggNames.length)];
  const rarities: ("Comum" | "Raro" | "Épico")[] = ["Comum", "Raro", "Épico"];
  const roll = Math.random();
  const rarity = roll < 0.5 ? "Comum" : roll < 0.85 ? "Raro" : "Épico";
  ...
  const newEgg: Pet = {
    ...
    rarity: rarity,
    active: false,
    color: rarity === "Épico" ? "#c084fc" : rarity === "Raro" ? "#60a5fa" : "#9ca3af",
  };

  s.pets.push(newEgg);
  logMsg(g, `🎉 OVO ADQUIRIDO: Você comprou um ${eggName} (${rarity})! Choque-o alimentando-o ou em combate.`, "#10b981");
