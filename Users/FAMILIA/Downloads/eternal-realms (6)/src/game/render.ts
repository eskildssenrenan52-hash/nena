...
import { drawWeatherOverlay } from "./ambitiousFeatures";
import { getContinentBiome } from "./continents";
...
function drawAmbitiousSystems(ctx: CanvasRenderingContext2D, g: GameState, camX: number, camY: number) {
  const amb = (g as any).ambitious;
  if (!amb) return;

  // 1. Render Custom Active Pet
  const customPet = amb.pets?.find((pt: any) => pt.active);
  if (customPet) {
    const p = g.player;
    const sx = p.pos.x - camX - 22;
    const sy = p.pos.y - camY + 8;
    ...
    // Pulse aura
    const pulseRad = 12 + Math.sin(g.time * 6) * 3;
    const grad = ctx.createRadialGradient(sx, sy + bob, 2, sx, sy + bob, pulseRad);
    grad.addColorStop(0, customPet.color + "99");
    grad.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.arc(sx, sy + bob, pulseRad, 0, Math.PI * 2);
    ctx.fill();

    // Pet sprite (emoji/symbol representing the pet)
    ...
    let petIcon = "🐾";
    if (customPet.type === "egg") petIcon = "🥚";
    else if (customPet.type === "fire") petIcon = "🔥";
    else if (customPet.type === "ice") petIcon = "❄️";
    else if (customPet.type === "lightning") petIcon = "⚡";
    else if (customPet.type === "healing") petIcon = "🧚";
    else if (customPet.type === "hybrid") {
      const hType = customPet.hybridType || "fire_ice";
      if (hType === "fire_ice") petIcon = "🔥❄️";
      else if (hType === "lightning_healing") petIcon = "⚡🧚";
      else if (hType === "fire_lightning") petIcon = "🔥⚡";
      else if (hType === "ice_healing") petIcon = "❄️🧚";
    }

    ctx.fillText(petIcon, sx, sy + bob);
    ctx.textBaseline = "alphabetic";

    // Draw pet level text
    ctx.fillStyle = "#fff";
    ctx.font = "bold 8px monospace";
    ctx.fillText(`Nv${customPet.level}`, sx, sy - 8 + bob);
  }

  // 1.5. Swirling Elemental Kill Combo Aura around the Player
  const comboCount = g.comboCount || 0;
  if (comboCount >= 3) {
    const p = g.player;
    const psx = p.pos.x - camX;
    const psy = p.pos.y - camY;
    const bob = Math.sin(g.time * 6) * 1.1;
    ctx.save();
    
    const comboColor = comboCount >= 15 ? "#ff007f" : comboCount >= 10 ? "#ff2244" : comboCount >= 7 ? "#ff6600" : "#ffaa00";
    const baseRadius = 18 + Math.min(12, comboCount * 0.5);
    const pulse = 1 + 0.12 * Math.sin(g.time * 8);
    const rad = baseRadius * pulse;
    
    ctx.strokeStyle = comboColor;
    ctx.lineWidth = comboCount >= 10 ? 2 : 1;
    ctx.shadowColor = comboColor;
    ctx.shadowBlur = Math.min(15, 4 + comboCount * 0.5);
    
    // Draw the main spinning glowing halo
    ctx.beginPath();
    ctx.arc(psx, psy + bob, rad, 0, Math.PI * 2);
    ctx.stroke();
    
    // Draw dual orbital sparks
    const angle1 = g.time * 5;
    const angle2 = g.time * 5 + Math.PI;
    const speedMult = 1 + Math.min(2, comboCount * 0.1);
    
    for (const angle of [angle1, angle2]) {
      const sparkX = psx + Math.cos(angle * speedMult) * rad;
      const sparkY = psy + bob + Math.sin(angle * sparkX * 0.01) * rad * 0.4; // slightly elliptical orbit
      ctx.fillStyle = "#fff";
      ctx.beginPath();
  ...
  // 2. Render Active Mercenaries
  if (amb.mercenaries) {
    for (const m of amb.mercenaries) {
      if (!m.active) continue;
      const sx = m.pos.x - camX;
      const sy = m.pos.y - camY;
      const bob = Math.sin(g.time * 5) * 2;

      // Shadow
      ctx.fillStyle = "rgba(0,0,0,0.35)";
      ctx.beginPath();
      ctx.ellipse(sx, sy + 14, 10, 3.5, 0, 0, Math.PI * 2);
      ctx.fill();
      ...
      ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.arc(sx, sy + 14, 11, 0, Math.PI * 2);
      ctx.stroke();

      // Body - colored circle for stylized look
      ctx.fillStyle = m.color;
      ctx.beginPath();
      ctx.arc(sx, sy + bob, 8, 0, Math.PI * 2);
      ctx.fill();

      // Role Icon inside body
      ctx.fillStyle = "#000";
      ctx.font = "bold 8px monospace";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ...
      // Mercenary Level & Name above head
      ctx.fillStyle = "#fff";
      ctx.font = "bold 9px monospace";
      ctx.fillText(`Lv${m.level} ${m.name.split(",")[0]}`, sx, sy - 12);

      // HP bar for mercenary
      const hpPct = m.hp / m.maxHp;
      const barW = 20;
      ctx.fillStyle = "rgba(0,0,0,0.6)";
      ctx.fillRect(sx - barW / 2, sy - 8, barW, 3);
      ctx.fillStyle = "#4ade80";
      ctx.fillRect(sx - barW / 2, sy - 8, barW * hpPct, 3);
    }
  }

  // 3. Render World Boss Malkor
  const wb = amb.worldBoss;
  if (wb && wb.active) {
    const sx = wb.pos.x - camX;
    const sy = wb.pos.y - camY;
    const t = g.time;
    const bob = Math.sin(t * 3) * 6;

    // Large Pulsing shadow
    ctx.fillStyle = "rgba(0,0,0,0.5)";
    ctx.beginPath();
    ctx.ellipse(sx, sy + 35, 35 + bob * 0.5, 12 + bob * 0.2, 0, 0, Math.PI * 2);
    ctx.fill();

    // Large Boss Body (Fierce crimson-orange core)
    ...
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.arc(sx, sy + bob, 27, 0, Math.PI * 2);
    ctx.stroke();

    // Boss symbol inside core
    ctx.fillStyle = "#fff";
    ...
    // Name & Health Bar above Boss
    ctx.fillStyle = "#f43f5e";
    ctx.font = "bold 12px monospace";
    ctx.fillText(wb.name.toUpperCase(), sx, sy - 42);

    const hpPct = Math.max(0, wb.hp / wb.maxHp);
    const barW = 80;
    ctx.fillStyle = "rgba(0,0,0,0.7)";
    ctx.fillRect(sx - barW / 2, sy - 34, barW, 6);
    ctx.fillStyle = "#ef4444";
    ctx.fillRect(sx - barW / 2, sy - 34, barW * hpPct, 6);
    ctx.strokeStyle = "#fbbf24";
    ctx.lineWidth = 1;
    ctx.strokeRect(sx - barW / 2, sy - 34, barW, 6);

    ctx.fillStyle = "#fff";
    ctx.font = "bold 8px monospace";
    ctx.fillText(`${Math.round(wb.hp)}/${wb.maxHp}`, sx, sy - 28);
    ...
    const lx = sx + Math.cos(wb.laserAngle) * 300;
    const ly = sy + bob + Math.sin(wb.laserAngle) * 300;
    ctx.lineTo(lx, ly);
    ctx.stroke();
...
function getBlendedBiomeColor(
  ...
  const size = mapObj.size || 100;
  if (!mapObj._blendedColorCacheArray) {
    mapObj._blendedColorCacheArray = new Array(size * size);
  }
  const idx = y * size + x;
  const cached = mapObj._blendedColorCacheArray[idx];
  ...
  mapObj._blendedColorCacheArray[idx] = result;
  return result;
}
...
function drawCityTile(
  ...
  else if (kind === 21) drawTemplePortal(ctx, sx, sy, time);
  else if (kind === 22) drawContinentsPortal(ctx, sx, sy, time);
  else if (kind === 23) drawMercenaryRecruiterNPC(ctx, sx, sy, time);
  else if (kind >= 24 && kind <= 30) drawContinentPortal(ctx, sx, sy, time, kind - 23);
  else if (kind === 31) drawUnifiedBiomePortal(ctx, sx, sy, time);
  else if (kind === 32) drawReturnPortal(ctx, sx, sy, time);
  else if (kind === 33) drawBiomeDungeonPortal(ctx, sx, sy, time, tileBiomeId);
}

function drawContinentsPortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number) {
  ctx.fillStyle = "#090514";
  ctx.fillRect(sx + 2, sy + 2, 28, 28);
  const cx = sx + 16, cy = sy + 16;
  const pulse = 0.85 + Math.sin(time * 6) * 0.15;
  for (let r = 12; r >= 3; r -= 3) {
    const angleOffset = r * 0.8 - time * 4.5;
    ctx.strokeStyle = r === 12 ? `rgba(168, 85, 247, ${pulse})` : r === 9 ? `rgba(59, 130, 246, ${pulse * 0.8})` : `rgba(244, 63, 94, ${pulse * 0.6})`;
    ctx.lineWidth = 2.0;
    ctx.beginPath();
    ctx.arc(cx, cy, r, angleOffset, angleOffset + Math.PI * 1.5);
    ctx.stroke();
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const rad = 8 + Math.sin(time * 4 + i) * 2;
    ctx.fillRect(cx + Math.cos(angle) * rad - 1, cy + Math.sin(angle) * rad - 1, 1.5, 1.5);
  }
...
function drawMercenaryRecruiterNPC(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number) {
  ctx.fillStyle = "#2d1f3d";
  ctx.fillRect(sx, sy, 32, 32);
  ctx.fillStyle = "#8b5cf6";
  ctx.fillRect(sx + 6, sy + 12, 20, 16);
  ...
  ctx.fillRect(sx + 24, sy + 4, 2, 24);
  ctx.fillStyle = "#ef4444";
  ctx.fillRect(sx + 20, sy + 6, 4, 8);
  ctx.fillStyle = "#fbbf24";
  ctx.fillRect(sx + 21, sy + 9, 2, 2);
  const bob = Math.sin(time * 3.5) * 1.5;
  ctx.fillStyle = "#fbbf24";
  ctx.font = "bold 9px sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("⚔️", sx + 16, sy - 3 + bob);
  ctx.textAlign = "left";
}

function drawContinentPortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number, continentId: number) {
  const colors = ["#10b981", "#fbbf24", "#f97316", "#ef4444", "#a855f7", "#06b6d4", "#ec4899"];
  const color = colors[continentId - 1] || "#fff";
  ...
  ctx.lineWidth = 1.8;
  ctx.beginPath();
  ctx.arc(cx, cy, 10, time * 2, time * 2 + Math.PI * 1.3);
  ctx.stroke();
  ctx.strokeStyle = "#fff";
  ctx.lineWidth = 1.0;
  ctx.beginPath();
  ctx.arc(cx, cy, 6, -time * 3, -time * 3 + Math.PI * 1.1);
  ctx.stroke();
  ctx.fillStyle = "#fff";
  ctx.font = "bold 8px monospace";
  ctx.textAlign = "center";
  ctx.fillText(`C${continentId}`, cx, cy + 3);
  ctx.textAlign = "left";
  ctx.strokeStyle = color;
  ctx.lineWidth = 1.8;
  ctx.strokeRect(sx + 2, sy + 2, 28, 28);
...
function drawUnifiedBiomePortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number) {
  ...
  ctx.lineWidth = 2.0;
  ctx.beginPath();
  ctx.arc(cx, cy, 11, -time * 4, -time * 4 + Math.PI * 1.5);
  ctx.stroke();
  ctx.fillStyle = "#e0f2fe";
  ctx.font = "bold 8px monospace";
  ctx.textAlign = "center";
  ctx.fillText("BIOMA", cx, cy + 3);
  ctx.textAlign = "left";
...
function drawReturnPortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number) {
  ...
  ctx.lineWidth = 2.0;
  ctx.beginPath();
  ctx.arc(cx, cy, 10, time * 3, time * 3 + Math.PI * 1.4);
  ctx.stroke();
  ctx.fillStyle = "#f1f5f9";
  ctx.font = "bold 7px monospace";
  ctx.textAlign = "center";
  ctx.fillText("SAÍDA", cx, cy + 2.5);
  ctx.textAlign = "left";
...
function drawBiomeDungeonPortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number, biomeId: number) {
  ...
  ctx.lineWidth = 2.0;
  ctx.beginPath();
  ctx.arc(cx, cy, 9, time * 3.5, time * 3.5 + Math.PI * 1.5);
  ctx.stroke();
  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 7px monospace";
  ctx.textAlign = "center";
  ctx.fillText(`P${biomeId}`, cx, cy + 2.5);
  ctx.textAlign = "left";
  ctx.strokeStyle = color;
  ctx.lineWidth = 1.8;
  ctx.strokeRect(sx + 2, sy + 2, 28, 28);
...
function drawTemplePortal(ctx: CanvasRenderingContext2D, sx: number, sy: number, time: number) {
  // Deep golden-red sacred portal
  ctx.fillStyle = "#1c1402";
  ctx.fillRect(sx + 2, sy + 2, 28, 28);

  const cx = sx + 16, cy = sy + 16;
  const pulse = 0.85 + Math.sin(time * 5) * 0.15;

  // Swirling holy energy rings
  for (let r = 12; r >= 4; r -= 2.5) {
    const angleOffset = r * 0.6 + time * 3.5;
    ctx.strokeStyle = r % 5 === 0 ? `rgba(255, 235, 120, ${pulse})` : `rgba(230, 160, 40, ${pulse * 0.7})`;
    ctx.lineWidth = 1.8;
    ctx.beginPath();
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const rad = 7 + Math.sin(time * 2.5 + i) * 1.5;
    const px = cx + Math.cos(angle) * rad;
    const py = cy + Math.sin(angle) * rad;
    ctx.fillRect(px - 1, py - 1, 2, 2);
  }
  ...
  glow.addColorStop(0, `rgba(230, 180, 50, ${0.4 * pulse})`);
  glow.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = glow;
  ctx.fillRect(sx - 8, sy - 8, 48, 48);

  ctx.strokeStyle = "#e6a028";
  ctx.lineWidth = 1.8;
  ctx.strokeRect(sx + 2, sy + 2, 28, 28);
}
...
function drawTempleAmbientOverlay(ctx: CanvasRenderingContext2D, g: GameState, sw: number, sh: number) {
  if (!g.activeTempleFloor || g.activeTempleFloor <= 0) return;

  ctx.save();
  // 1. Holy Golden Sun Rays / God Rays
  ctx.globalCompositeOperation = "lighter";
  ctx.fillStyle = "rgba(255, 215, 0, 0.08)";
  const t = g.time * 0.3;
  for (let i = 0; i < 4; i++) {
    const startX = (((i * 0.28 + t * 0.05) % 1.2) - 0.2) * sw;
    ctx.beginPath();
    ctx.moveTo(startX, 0);
    ctx.lineTo(startX + 140, 0);
    ctx.lineTo(startX - 60, sh);
    ctx.lineTo(startX - 200, sh);
    ctx.closePath();
    ctx.fill();
  }

  // 2. Falling Sacred Golden Sparkles / Feathers
  ctx.fillStyle = "rgba(255, 235, 120, 0.75)";
  const particleCount = 25;
  for (let i = 0; i < particleCount; i++) {
    const seed = i * 53.9;
    const x = (seed + g.time * 16) % sw;
    const y = (seed * 19.7 + g.time * 35) % (sh + 40) - 20;
    const size = 1.6 + (i % 3) * 1.3;
    const sway = Math.sin(g.time * 0.9 + seed) * 24;

    ctx.beginPath();
    ctx.arc(x + sway, y, size, 0, Math.PI * 2);
    ctx.fill();

    // Minor glowing halo around sparkle
    ctx.fillStyle = "rgba(255, 215, 0, 0.2)";
    ctx.beginPath();
    ctx.arc(x + sway, y, size * 2.5, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "rgba(255, 235, 120, 0.75)";
  }
...
export function render(ctx: CanvasRenderingContext2D, g: GameState, w: number, h: number) {
  ...
  for (let y = y0; y < y1; y++) {
    for (let x = x0; x < x1; x++) {
      ...
      if (inGrand) {
        ...
        } else if (t.kind >= 8 && t.kind <= 33) drawCityTile(ctx, sx, sy, t.kind, x, y, g.time, t.biome);
        else {
          drawGrassTile(ctx, sx, sy, t.biome, x, y, g.time, blendedCol);
        ...
        } else if (t.kind >= 8 && t.kind <= 33) {
          drawCityTile(ctx, sx, sy, t.kind, x, y, g.time, t.biome);
        } else if (t.kind === 6 || t.kind === 7) {
          drawStair(ctx, sx, sy, t.kind, g.time, t.biome);
  ...
  drawAmbitiousSystems(ctx, g, camX, camY);
  ...
  // Magnificent Holy Golden Glow inside the Temple
  drawTempleAmbientOverlay(ctx, g, sw, sh);
  ...
  if (g.ambitious) {
    drawWeatherOverlay(ctx, g.ambitious, w, h, g.time);
  }
