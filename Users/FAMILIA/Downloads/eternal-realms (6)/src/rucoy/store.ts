...
import { ALL_ITEMS, type Item, type ItemSlot } from "./items";
...
export interface OverlayState {
  ...
  // Recursos de sobrevivência / navegação (Ilha 2)
  wood: number;
  hasBoat: boolean;
  island: number;
  shards?: number;
  blacksmithLvl?: number;
  playerBio?: string;
  isSailingLoading?: boolean;
  sailingLoadingProgress?: number;
  sailingLoadingTip?: string;
  sailingTargetIsland?: number | null;
}

const DEFAULT: OverlayState = (() => {
  // O jogador começa SEM nenhum item no inventário — precisa conquistar tudo.
  return {
    inventory: [],
    equipped: {},
    unlockedPets: [PETS[0].id],
    activePet: PETS[0].id,
    claimedFree: [],
    claimedPremium: [],
    hasPremium: false,
    ...
    hasBoat: false,
    island: 1,
    playerBio: "Herói destemido de Asterion",
    customization: {
...
export function setOverlayState(
  patch: Partial<OverlayState> | ((s: OverlayState) => OverlayState),
) {
...
// ============ Sistema de Madeira e Barco (Ilha 2) ============
export const BOAT_WOOD_COST = 50;

export function addWood(amount: number) {
  setOverlayState((s) => ({ ...s, wood: Math.max(0, s.wood + amount) }));
}

export function craftBoat(): boolean {
  const s = getOverlayState();
  if (s.hasBoat) return false;
  if (s.wood < BOAT_WOOD_COST) return false;
  setOverlayState((cs) => ({ ...cs, wood: cs.wood - BOAT_WOOD_COST, hasBoat: true }));
  return true;
}

export function setIsland(island: number) {
  setOverlayState((s) => ({ ...s, island }));
}
