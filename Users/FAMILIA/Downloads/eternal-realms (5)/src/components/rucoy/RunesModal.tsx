...
export default function RunesModal() {
  ...
  const [selectedRuneUid, setSelectedRuneUid] = useState<string | null>(null);
  ...
  // Forge calculation and upgrade handlers
  const selectedRune = (p.runes || []).find((r) => r.uid === selectedRuneUid);
  const selectedInfo = selectedRune ? ANCIENT_RUNES[selectedRune.runeId] : null;

  const getUpgradeCost = (tier: AncientRuneTier) => {
    if (tier === 1) return 50000;
    if (tier === 2) return 250000;
    return 0;
  };

  const cost = selectedRune ? getUpgradeCost(selectedRune.tier) : 0;
  const canForge = selectedRune && selectedRune.tier < 3 && p.gold >= cost;

  const handleForge = () => {
    if (!selectedRune || !canForge) return;
    p.gold -= cost;
    selectedRune.tier = (selectedRune.tier + 1) as AncientRuneTier;
    setForgeSuccess(true);
    applyChange();
    logMsg(g, `⚒️ FORJA: Sua runa ${selectedInfo?.name} foi aprimorada para o Grau ${runeTierLabel(selectedRune.tier)}!`, "#fbbf24");
    setTimeout(() => {
  ...
  return (
    <div style={{ fontFamily: PIXEL_FONT, color: "#e8d8a8", fontSize: 10 }}>
      ...
      <PixelPanel tone="parchment" style={{ marginBottom: 14 }}>
        ...
        {(() => {
          const activeRunes = equipped.filter((r): r is OwnedRune => !!r);
          const activeSynergies = getActiveSynergies(activeRunes);
          ...
          return (
            ...
            >
              <div style={{ color: "#d8b4fe", fontSize: 8, fontWeight: "bold", letterSpacing: 1, marginBottom: 6, textAlign: "center" }}>
                ✨ SINERGIAS RÚNICAS ATIVAS ✨
              </div>
              {activeSynergies.map((syn, idx) => (
                <div key={idx} style={{ marginBottom: idx < activeSynergies.length - 1 ? 6 : 0, borderBottom: idx < activeSynergies.length - 1 ? "1px solid rgba(168, 85, 247, 0.15)" : "none", pb: idx < activeSynergies.length - 1 ? 4 : 0 }}>
                  <div style={{ color: "#fef08a", fontSize: 8, fontWeight: "bold" }}>⚡ {syn.name}</div>
                  <div style={{ color: "#d6d3d1", fontSize: 7, marginTop: 2, lineHeight: 1.3 }}>{syn.desc}</div>
                </div>
              ))}
            </div>
          );
      ...
      {/* Mystical Rune Forge Panel */}
      <PixelPanel
        tone="parchment"
        style={{
          marginBottom: 14,
          background: "linear-gradient(135deg, rgba(23, 11, 41, 0.95) 0%, rgba(13, 8, 22, 0.95) 100%)",
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ color: PIXEL_GOLD, letterSpacing: 2, fontWeight: "bold" }}>FORJA DE RUNAS ANCESTRAIS</span>
          </div>
          <span style={{ fontSize: 8, color: "#a78bfa", fontWeight: "bold" }}>SINK DE OURO</span>
        </div>

        {selectedRune && selectedInfo ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(0,0,0,0.4)", padding: 8, borderRadius: 4 }}>
              <RuneChip rune={selectedRune} size={54} glow />
              <div style={{ flex: 1 }}>
                <div style={{ color: selectedInfo.color, fontSize: 10, fontWeight: "bold" }}>{selectedInfo.name}</div>
                <div style={{ fontSize: 8, color: "#d1d5db", marginTop: 2 }}>
                  Grau Atual: <span style={{ color: TIER_GLOW[selectedRune.tier], fontWeight: "bold" }}>{runeTierLabel(selectedRune.tier)} (T{selectedRune.tier})</span>
                </div>
                {selectedRune.tier < 3 ? (
                  <div style={{ fontSize: 8, color: "#a78bfa", marginTop: 2 }}>
                    Próximo Grau: <span style={{ color: TIER_GLOW[(selectedRune.tier + 1) as AncientRuneTier], fontWeight: "bold" }}>{runeTierLabel((selectedRune.tier + 1) as AncientRuneTier)} (T{selectedRune.tier + 1})</span>
                  </div>
                ) : (
                  <div style={{ fontSize: 8, color: "#34d399", marginTop: 2, fontWeight: "bold" }}>✨ Grau Supremo Máximo!</div>
                )}
              </div>
            </div>

            {selectedRune.tier < 3 ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(0,0,0,0.2)", padding: "6px 8px" }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <div style={{ fontSize: 7, color: "#9ca3af" }}>CUSTO DE CRISTALIZAÇÃO:</div>
                  <div style={{ fontSize: 9, color: p.gold >= cost ? "#facc15" : "#f87171", fontWeight: "bold", display: "flex", alignItems: "center", gap: 3 }}>
                    🪙 {cost.toLocaleString("pt-BR")} Gold
                    {p.gold < cost && <span style={{ fontSize: 7, fontWeight: "normal" }}> (Insuficiente)</span>}
                  </div>
                </div>
                <PixelButton
                  variant="gold"
                ...
                >
                  {forgeSuccess ? "⚡ CRISTALIZANDO... ⚡" : "⚒️ APRIMORAR RUNA"}
                </PixelButton>
              </div>
            ) : (
              <div style={{ textAlign: "center", padding: 8, color: "#9ca3af", fontSize: 8 }}>
                Esta runa já atingiu o potencial supremo concedido pelos arcontes de Asterion!
              </div>
            )}
          </div>
        ) : (
          <div style={{ textAlign: "center", padding: "14px 10px", color: "#9ca3af", fontSize: 8, background: "rgba(0,0,0,0.3)" }}>
            Selecione uma runa do seu inventário abaixo para iniciar o processo de forja e aprimorar seus atributos ancestrais!
          </div>
        )}
      </PixelPanel>
          ...
          >
            {inventoryRunes.map((rune) => {
              ...
              return (
                <div
                  key={rune.uid}
                  onClick={() => setSelectedRuneUid(rune.uid)}
                  style={{
                    background: selectedRuneUid === rune.uid ? "rgba(234, 179, 8, 0.08)" : "rgba(0,0,0,0.55)",
                    border: selectedRuneUid === rune.uid ? `2px solid ${PIXEL_GOLD}` : `2px solid ${info.color}55`,
                    padding: 8,
                    display: "flex",
                    gap: 10,
                    alignItems: "flex-start",
                    cursor: "pointer",
                    boxShadow: selectedRuneUid === rune.uid ? `0 0 10px ${PIXEL_GOLD}55` : "none",
                  }}
                >
                  <RuneChip
                    rune={rune}
