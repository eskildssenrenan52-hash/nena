...
import { getSkinSlug, drawEclipseEternoSkin } from "@/game/render";
import { getSpriteBySlug } from "@/game/sprites";
import { getCustomOptionsForCategory } from "./CustomizationModal";
import { ALL_ITEMS, SLOT_ICON, SLOT_LABEL, RARITY_COLOR, RARITY_BG, type ItemSlot } from "@/rucoy/items";
...
function PlayerPreviewCanvas({ cls, skin }: { cls: string; skin: number }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (skin === 7) {
        drawEclipseEternoSkin(ctx, null, 32, 32, bob, Date.now() / 1000, 1, 0, cls);
      } else {
...
export default function ProfileModal() {
  ...
  const [bioInput, setBioInput] = useState(state.playerBio || "");
  ...
  const saveBio = () => {
    setOverlayState((cs) => ({ ...cs, playerBio: bioInput }));
    setEditingBio(false);
    showFeedback(lang === "pt" ? "Bio atualizada!" : "Bio updated!");
  };

  const doResetAttrs = () => {
    const cost = 500;
    if (p.gold < cost) {
      showFeedback(lang === "pt" ? "Você precisa de 500 de Ouro!" : "You need 500 Gold!");
      return;
    }

    // Calculate total spent points (attributes default to 5)
    const spentStr = Math.max(0, p.attr.str - 5);
    const spentAgi = Math.max(0, p.attr.agi - 5);
    const spentInt = Math.max(0, p.attr.int - 5);
    const spentVit = Math.max(0, p.attr.vit - 5);
    const totalSpent = spentStr + spentAgi + spentInt + spentVit;

    if (totalSpent <= 0) {
      showFeedback(lang === "pt" ? "Nenhum ponto para resetar!" : "No points to reset!");
      return;
    }
    ...
    recalculateStats(g);
    bumpVersion();
    showFeedback(lang === "pt" ? "Atributos resetados! 🛡️" : "Attributes reset! 🛡️");
  };
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 3, minWidth: 0 }}>
          ...
          {/* Dynamic Bio status */}
          <div style={{ borderTop: "1px dashed rgba(255,255,255,0.1)", paddingTop: 3, marginTop: 1 }}>
            {editingBio ? (
              <div style={{ display: "flex", gap: 4, width: "100%" }}>
                ...
                />
                <PixelButton onClick={saveBio} size={4} variant="gold" style={{ padding: "1px 4px" }}>
                  ✓
                </PixelButton>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 4 }}>
                <span style={{ fontSize: 7, color: "#ccc", fontStyle: "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  "{state.playerBio || (lang === "pt" ? "Mudar status..." : "Edit bio...")}"
                </span>
                <button
                  onClick={() => { setEditingBio(true); setBioInput(state.playerBio || ""); }}
                  style={{
      ...
      <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
        {activeTab === "attributes" ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {/* Points Summary with Reset Button */}
            <div
              style={{
                display: "flex",
            ...
            >
              ...
              <PixelButton
                variant="gold"
                size={5}
                style={{ fontSize: 6.5, padding: "2px 6px" }}
                onClick={doResetAttrs}
              >
                RESETAR (500g)
              </PixelButton>
            </div>
            ...
            {/* Secondary Attributes and CP score */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
              <div style={{ background: "rgba(0,0,0,0.35)", padding: 6, borderRadius: 4, display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={{ color: "#eab308", fontWeight: "bold", fontSize: 7 }}>ATRIBUTOS SECUNDÁRIOS</span>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>CHANCE DE CRÍTICO:</span>
                  <span style={{ color: "#22c55e", fontWeight: "bold" }}>{(5 + Math.round(p.attr.agi * 0.15 * 10) / 10).toFixed(1)}%</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>REGEN DE VIDA:</span>
                  <span style={{ color: "#ef4444", fontWeight: "bold" }}>{(1 + Math.round(p.attr.vit * 0.1 * 10) / 10).toFixed(1)}/s</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>REGEN DE MANA:</span>
                  <span style={{ color: "#3b82f6", fontWeight: "bold" }}>{(1 + Math.round(p.attr.int * 0.1 * 10) / 10).toFixed(1)}/s</span>
                </div>
              </div>

              <div style={{ background: "rgba(0,0,0,0.35)", padding: 6, borderRadius: 4, display: "flex", flexDirection: "column", gap: 3 }}>
                <span style={{ color: "#38bdf8", fontWeight: "bold", fontSize: 7 }}>PONTUAÇÃO DE BATALHA</span>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>PODER TOTAL (CP):</span>
                  <span style={{ color: "#fff", fontWeight: "bold" }}>{p.level * 10 + p.atk() * 5 + p.def() * 5} CP</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>PRESTÍGIO:</span>
                  <span style={{ color: "#eab308", fontWeight: "bold" }}>{Math.floor((p.level + (p.attr.str + p.attr.agi + p.attr.int + p.attr.vit) / 4) / 5)}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 6.5 }}>
                  <span style={{ color: "#ccc" }}>TROFÉUS:</span>
                  <span style={{ color: "#a855f7", fontWeight: "bold" }}>{state.trophies || 0}</span>
                </div>
              </div>
            </div>

            {/* Active Equipment Slots Strip */}
            <div style={{ background: "rgba(0,0,0,0.4)", padding: 6, borderRadius: 6, border: "1.5px solid rgba(255,255,255,0.05)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <span style={{ color: PIXEL_GOLD, fontWeight: "bold", fontSize: 7.5 }}>EQUIPAMENTO ATIVO</span>
                <span style={{ fontSize: 6, color: "#8a9ba8" }}>PASSE O MOUSE PARA ATRIBUTOS</span>
              </div>
              
              <div style={{ display: "flex", gap: 3, overflowX: "auto", paddingBottom: 2 }}>
                {(["helm", "chest", "legs", "boots", "weapon", "shield", "cape"] as ItemSlot[]).map((slot) => {
                  const itemId = state.equipped[slot];
                  const item = itemId ? ALL_ITEMS.find((it) => it.id === itemId) : null;
                  const icon = SLOT_ICON[slot];
                  const name = item ? item.name : (lang === "pt" ? "Vazio" : "Empty");
                  const rarityColor = item ? RARITY_COLOR[item.rarity] : "rgba(255,255,255,0.1)";
                  const rarityBg = item ? RARITY_BG[item.rarity] : "rgba(0,0,0,0.25)";
                  
                  return (
                    <div
                      key={slot}
                      title={item ? `${name} (Nvl ${item.level}) [Atk: ${item.attack} / Def: ${item.defense}]` : (lang === "pt" ? `${SLOT_LABEL[slot]}: Vazio` : `${slot.toUpperCase()}: Empty`)}
                      style={{
                        flexShrink: 0,
                        width: 28,
                        height: 28,
                    ...
                    >
                      ...
                      <span style={{ fontSize: 4, color: "#fff", position: "absolute", bottom: 1, textTransform: "uppercase", scale: "0.8" }}>
                        {item ? item.rarity.substring(0, 3) : slot.substring(0, 4)}
                      </span>
                      {item && (
                        <div style={{
                          position: "absolute",
