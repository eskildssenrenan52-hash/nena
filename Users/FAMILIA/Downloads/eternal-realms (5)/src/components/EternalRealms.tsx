...
export default function EternalRealms() {
  ...
  return (
    ...
    >
      ...
      {/* 🔮 BARRA DA HABILIDADE SUPREMA (ULTIMATE) */}
      <div 
        className="fixed z-20 flex flex-col items-center gap-1.5 pointer-events-auto bg-black/85 border border-purple-600/50 p-2.5 rounded-lg shadow-2xl backdrop-blur-sm"
        style={{ bottom: "24px", left: "50%", transform: "translateX(-50%)", minWidth: "220px" }}
      >
        <div className="flex justify-between items-center w-full text-[10px] font-mono text-purple-300 font-bold">
          <span>🔮 FÚRIA DOS DEUSES</span>
          <span className="text-purple-400">Teclado [R]</span>
        </div>
        <div className="w-full bg-neutral-900 border border-neutral-700 h-4 rounded overflow-hidden relative">
          <div 
            className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 h-full transition-all duration-300 relative"
            style={{ width: `${Math.min(100, (stateRef.current?.ambitious?.ultimateGauge ?? 0))}%` }}
          >
            {/* Pulsing light effect inside the gauge */}
            <div className="absolute inset-0 bg-white/20 animate-pulse"></div>
          </div>
          <div className="absolute inset-0 flex items-center justify-center text-[10px] font-mono text-white font-bold drop-shadow">
        ...
        <button
          ...
          className={`w-full text-[10px] py-1 font-mono font-bold rounded tracking-widest uppercase transition-all duration-200 active:scale-95 ${
            (stateRef.current?.ambitious?.ultimateGauge ?? 0) >= 100
              ? "bg-purple-600 hover:bg-purple-500 text-white animate-bounce cursor-pointer shadow-[0_0_15px_#a855f7]"
              : "bg-neutral-800 text-neutral-500 cursor-not-allowed border border-neutral-700/50"
          }`}
        >
          {(stateRef.current?.ambitious?.ultimateGauge ?? 0) >= 100 ? "💥 EXECUTAR SUPREMO 💥" : "Carregando Fúria..."}
        </button>
      </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {amb.pets?.map((pet: any) => {
                      let desc = "";
                      let icon = "🐾";
                      if (pet.type === "egg") { desc = "Choque matando monstros!"; icon = "🥚"; }
                      ...
                      const progress = (pet.xp / pet.nextLvlXp) * 100;
                        ...
                        >
                          ...
                          <div className="mt-2.5">
                            <div className="flex justify-between text-[8px] text-purple-400/90 font-bold mb-1">
                              ...
                              <span>{Math.round(pet.xp)}/{pet.nextLvlXp}</span>
                            </div>
