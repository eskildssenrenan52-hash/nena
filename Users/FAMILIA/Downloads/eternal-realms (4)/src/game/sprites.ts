...
const URL_PREFIX = BASE_URL.endsWith("/") ? BASE_URL : BASE_URL + "/";

function resolveUrl(path: string): string {
  const cleanPath = path.startsWith("/") ? path.slice(1) : path;
  const origin = typeof window !== "undefined" ? window.location.origin : "";
  return origin + URL_PREFIX + cleanPath;
}
...
function loadFirstAvailableSprite(candidates: string[]): HTMLImageElement | null {
  ...
  if (!e) {
    ...
    const attempts = Array.from(new Set(candidates.flatMap(getAttemptsForSlug))).map(resolveUrl);
    let idx = 0;
    img.onerror = () => {
      idx += 1;
